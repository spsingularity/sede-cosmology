...
% Retargeted from template_cosmology.tex: JCAP requires an arXiv identifier
...
\begin{abstract}
We present Structural Entropy Dark Energy (SEDE), in which the
...
of cosmic structure --- a dark sector with \textbf{zero fitted
parameters and three stated prescriptions}: the amplitude is fixed by
flatness, the gate shape by a halo-binding prescription
\((\gamma  \approx  1.5)\), the H-scaling by the volume-law postulate
\((\lambda  = 1 - \Delta /2 = 1/2)\), and the sound speed by the closure
\(c_{\mathrm{s}}^{2} = 1\). Nothing is tuned to the data; each
prescription is a stated choice, declared before the fit, and the one
discrete postulate is a volume-law (Barrow \(\Delta  = 1\)) horizon
entropy, the quantity our forecasts target. To be explicit about scope,
SEDE modifies \emph{only} the dark-energy density in an otherwise
standard-GR Friedmann background (holographic DE, not modified gravity),
with \(\rho _\mathrm{DE} = T_\mathrm{AH}\) \(s_{\mathrm{grav}}\)
...
\begin{keyword}
dark energy theory \sep dark energy experiments \sep cosmological perturbation
theory \sep modified gravity
\end{keyword}
...
deformation enters \(\rho _\mathrm{DE}\) alone.) We stress that this is
a \emph{substantive} restriction rather than a stylistic one, and that
the alternative is disfavoured rather than merely different: feeding the
same deformation into the field equations makes the scalar sector modify
gravity directly, and a ghost-free, gradient-stable completion with
\(c_{\mathrm{T}} = 1\) then forces the effective coupling upward --- an
enhancement of structure growth larger than the present data permit. The
asymmetry between deforming \(\rho _\mathrm{DE}\) and deforming the
geometry is therefore load-bearing: it is the branch of the construction
that survives the growth constraints, not simply the branch we chose to
develop. Correspondingly, \(\rho _\mathrm{DE} = T_\mathrm{AH}\)
\(s_{\mathrm{grav}}\) \(f_{\mathrm{sat}}\) is imposed as an \emph{exact
effective relation} that fixes the dark-energy density's amplitude and
its H-scaling; the equation of state is then a derived, kinematic
consequence of energy conservation on that standard background (it
crosses \(w = -1\) by \emph{sourcing}), not the equilibrium enthalpy
relation \(\rho  + p = Ts\), which does not hold for this open,
structure-fed sector. The whole construction is governed by one
bookkeeping rule --- that energy is conserved when this fluid's redshift
dependence is gated by structure growth. We first state the model (the
conjugate horizon-fluid ansatz, the structure gate, the Friedmann
equation, and how \(\Lambda\) is replaced), and then make energy
...
shows the \(w = -1\) crossing to be sourcing rather than a phantom
field.
...
sector and it is what enters the calibrated E(z). We emphasise that this
selection is made on the physical argument alone and does not depend on
the outcome: the \((1 - \epsilon /2)\) correction is parametrically
negligible precisely where this fluid carries any weight, so the
\(\epsilon\) → 0 limit would be the right leading choice for a
slowly-evolving horizon fluid \emph{even had the dynamical variant
produced an acceptable background}. That the dynamical variant
additionally fails is a consistency check on the choice, not its
justification. We are nonetheless explicit about the epistemic status of
this step: the Gibbons--Hawking form is a convention, and one whose
adoption is corroborated by the failure of the dynamical alternative, so
quantities downstream of it --- notably the \(w = -1\) crossing redshift
(§\ref{sec-6}) --- are convention-conditional outputs of the adopted
temperature, not convention-free predictions. We \emph{identify} the
dark-energy density with the conjugate horizon-fluid energy density,
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../../results/paper/fig1_mechanism}}
...
\section{Fixed inputs: zero fitted parameters, three stated
prescriptions}\label{sec-3}
...
\texttt{results/\allowbreak predictions.\allowbreak json}. The CPL \(w_{\mathrm{a}}\) steepens
if fit over a wider range, \(e.g. -0.17\) over 0 \textless{} z
\textless{} 2, so the fit window is quoted with the pair.)} Three
\(w_{0}\) values appear in this paper for \emph{different} objects; to
avoid confusion:
...
(\texttt{c\_\allowbreak s²=1}) limit of the completed theory rather than the theory
itself.
...
\texttt{src/\allowbreak experiments/\allowbreak tensor\_\allowbreak sector/\allowbreak fpab\_\allowbreak tensor\_\allowbreak sector.\allowbreak py}). Its
perturbations are solved \emph{dynamically} (Mochi-CLASS, not a PPF or
an imposed sound speed): ghost- and gradient-stable, with
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../../results/paper/fig2_inputs}}
...
(pre-registration §\ref{sec-E}): the \texttt{act\_\allowbreak dr6\_\allowbreak mflike}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../../results/paper/fig3_datafit}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../../results/paper/fig4_eos}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../../results/fig_cmb_earlyde}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../../results/fig_F5_calibration}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../../results/paper/fig5_oos}}
...
\texttt{src/\allowbreak experiments/\allowbreak run\_\allowbreak model\_\allowbreak comparison.\allowbreak py} in the reproduction
repository.
...
force) \\
\(E_G\) (lensing-to-velocity ratio) & \textbf{+3.4\% at z = 0}, +2.1\%
at z = 0.3, +0.5\% at z = 1 \((\Sigma  = \mu  = 1.05\) against growth
+1.6\%); \emph{enhanced}, never suppressed & \(\Lambda \mathrm{CDM}\)
(\(E_G\) = \(\Omega\)\(_{m,0}\)/f); a suppressed \(E_G\) falsifies \\
...
prediction set is committed as \texttt{results/\allowbreak predictions.\allowbreak json} in the
reproduction repository, cryptographically locked by
\texttt{sha256(predictions.\allowbreak json)} =
...
it is regenerated byte-for-byte by
\texttt{src/\allowbreak scripts/\allowbreak gen\_\allowbreak predictions.\allowbreak py}, so the registered values
cannot be altered after this digest is published.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../../results/paper/fig6_forecast}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../../results/fig_e1_mechanism}}
...
smooth dark energy \emph{at the background level}; its perturbation
sector is the cubic-KGB completion, which has zero gravitational slip
but \(\mu _\infty  = 1.05\). Zero slip fixes \(\Phi  = \Psi\) and
therefore \textbf{\(\Sigma  = \mu\)} --- it does \emph{not} set
\(\Sigma  = 1\), and \(E_{\mathrm{G}} = \Omega _{\mathrm{m}}\),0
\(\Sigma /f\) moves whenever \(\mu\) and the growth rate fail to cancel.
Running the FPAB-SEDE completion through Mochi-CLASS
(\texttt{src/\allowbreak experiments/\allowbreak run\_\allowbreak eg\_\allowbreak consistency.\allowbreak py}) settles both halves:
the slip \(\Phi /\Psi  = 1.00000\) at every redshift, and
\(\Sigma /\mu  = 1.00000\) with \(\Sigma  = \mu  = 1.051\) at z = 0 ---
reproducing the independently computed \(\mu _\infty\) of the
observational-tests companion through a different observable channel.
The growth rate is enhanced by only 1.6\%, so the cancellation is
partial:

\(E_{\mathrm{G}}(\mathrm{SEDE})/E_{\mathrm{G}}(\Lambda \mathrm{CDM}) = \mu\)
· \(f_\mathrm{GR}/f_\mathrm{SEDE}\)=\textbf{1.034 at z = 0}, falling to
1.021 at z = 0.3 and 1.005 at z = 1.

\(E_{\mathrm{G}}\) is therefore \textbf{not} a shared GR null, as an
earlier version of this section claimed. It is a second parameter-free
consequence of \(\mu _\infty  > 1\), in the same enhancing direction as
the \(f\sigma 8\) prediction. \(At \leq 3.4\)\% it still sits below the
∼5--20\% current \(E_{\mathrm{G}}\) errors, so it remains consistent
with the DESI-DR1 + weak-lensing measurement to \(z \approx  1\)
\citep{Rauhut:2025eaz} --- a target for future surveys rather than a
present discriminator, and a \emph{signed} one: SEDE forbids a
suppressed \(E_{\mathrm{G}}\). The sharpest discriminator remains the
deformation amplitude \(\Delta\) (§\ref{sec-5-6}, §\ref{sec-6}). Failure
of the lock (P2) would falsify the structure-sourcing mechanism itself
--- SEDE would then be an ordinary kinematic w(z) --- so P2 is the test
that separates the mechanism from a curve fit.
...
symbolically in
\texttt{src/\allowbreak experiments/\allowbreak tensor\_\allowbreak sector/\allowbreak fpab\_\allowbreak tensor\_\allowbreak sector.\allowbreak py}
...
\texttt{src/\allowbreak experiments/\allowbreak tensor\_\allowbreak sector/\allowbreak fig\_\allowbreak xi0\_\allowbreak w0.\allowbreak py} in the
repository.
...
\texttt{src/\allowbreak experiments/\allowbreak tensor\_\allowbreak sector/\allowbreak prerecomb\_\allowbreak earlyde.\allowbreak py},
consistent with the \(\Omega_{\rm DE}(z=3) = 0.029\) of Fig. 5). SEDE
therefore carries no early dark energy and the sound horizon \(r_s\) is
exactly standard --- the model bets entirely on the late-time side of
the Hubble tension; a confirmed percent-level EDE detection would
falsify the gate as the sole mechanism. And since \(\alpha_M = 0\) holds
at \emph{all} epochs (background-EOM-free), the primordial tensor
transfer function is exactly GR: any future feature in CMB \(B\)-modes
or the primordial stochastic GW background is attributable to sources,
never to SEDE propagation.
...
\textbf{Background robustness (model-independent H(z)).} Against the
standard 31-point cosmic-chronometer compilation --- the cleanest
model-independent expansion-history data, with no sound-horizon
calibration --- the model fits with \(\Delta \chi ^{2} = +0.04\)
relative to flat \(\Lambda \mathrm{CDM}\) \emph{at equal parameter
count} \((\Omega _{\mathrm{m}} = 0.325\), \(H_{0} = 67.9\) vs 0.320,
68.1 km \(s^{-1}\) \(Mpc^{-1}\); \(\gamma\) fixed at its halo-statistics
value throughout; script
\texttt{src/\allowbreak experiments/\allowbreak run\_\allowbreak deficit\_\allowbreak data.\allowbreak py}). The background is
indistinguishable from \(\Lambda \mathrm{CDM}\) on H(z) alone: the
gate's imprint on the \(z \geq  0\) expansion shape is absorbed by an
\((\Omega _{\mathrm{m}}\), \(H_{0}\)) shift to the \textasciitilde0.2\%
level (an \(o\Lambda \mathrm{CDM}\) fit to the model's own curve returns
\(\Omega _{\mathrm{k}}\),eff = 0.000) --- which is precisely why the
model's discriminating power resides in the growth and tensor sectors
(§\ref{sec-6}) rather than in the background.

\textbf{The deep-future second law selects a saturation endpoint.}
Extending the background beyond the fitted range exposes a structural
constraint invisible to data (script
\texttt{src/\allowbreak experiments/\allowbreak run\_\allowbreak gate\_\allowbreak selection.\allowbreak py}). On the accelerating
attractor the gate's remaining fill decays at the linear-growth rate
\(\propto  e^{-2N}\) (N = ln a), slower than matter's \(e^{-3N}\), so
any gate still strictly filling at the attractor forces H to undershoot
\(H_\infty\) and the horizon entropy to overshoot its ceiling
transiently --- a sub-percent \((\approx 0.25\)\%), strictly future
\((a \gtrsim  6.5)\) violation of the generalized second law. No theorem
is contradicted: the null energy condition fails there transiently,
which is exactly where GSL proofs do not apply, and the entire
observable past is monotone. But for a model whose fundamental object
\emph{is} horizon entropy, requiring the second law globally is natural,
and it bounds the saturation endpoint: the gate must complete its
filling at x* ≲ 1.8, where
\(x = \sigma _{8}^{2}(z)/\sigma _{8}^{2}(0)\). Every member of the
capped family x* ∈ {[}1, 1.8{]} is identical to the uncapped exponential
gate over \(z \geq  0\) to machine precision --- the bound carries no
observational cost --- and its maximally conservative member (x* = 1)
places completion at the present epoch, an independent, sharper
restatement of the coincidence problem. (The Bousso-bound hypothesis H5,
stated in full-saturation units, is automatically satisfied by any CDF
gate and is not this bound; the second-law cap is a new, strictly
stronger statement.)
...
\item
  ...
  \texttt{act\_\allowbreak dr6\_\allowbreak mflike} + multi-GB products not installed), with the
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../../results/fig_delta_orthogonality}}
...
\href{https://doi.org/10.5281/zenodo.21525524}{10.5281/zenodo.21525524}.
...
(\texttt{results/\allowbreak predictions.\allowbreak json}, verified byte-for-byte by
\texttt{src/\allowbreak scripts/\allowbreak gen\_\allowbreak predictions.\allowbreak py}). A single entry point
(\texttt{reproduce.\allowbreak py}) regenerates the figures, and the inference
drivers under \texttt{src/\allowbreak experiments/\allowbreak } regenerate the headline
model-comparison numbers; an automated verification suite checks the
background layer numerically. Two components are \emph{not} regenerated
locally and are reported from the companion observational-tests analysis
\citep{Pandev:2026obstest}: the full-primary-CMB (plik) joint
\((\Delta \mathrm{DIC} = -3.5\), robust part \(\Delta\)⟨\(\chi ^{2}\)⟩ ≈
−3.0, eroded to \(-2.3\) by the companion's direct Planck-lensing
\(\phi \phi\) test; this paper's own fold-in
\(\Delta \chi ^{2} = -3.17\)), which needs the Planck likelihood data,
and the FPAB-SEDE cubic-KGB / Mochi-CLASS growth sector \((\sigma 8\),
\(\mu _\infty\), the ISW ratio, gravitational slip); the nested-sampling
evidence is reported via a Laplace estimate here (full dynesty run in
the companion). The observational inputs are third-party public datasets
(DESI DR2, Pantheon+/DES-5YR/Union3, Moresco cosmic chronometers,
Gold-2018 \(f\sigma 8\), compressed Planck, eBOSS DR16), available at
their cited original sources and retrieved via the documented loaders.
...
During the preparation of this work the author used Claude Opus 4.x
(Anthropic) in order to draft and edit the manuscript, to develop and
cross-check the analysis code, and to assist with literature searches.
After using this tool the author reviewed and edited the content as
needed --- including checking every literature reference against its
source --- and takes full responsibility for the content of the
published article. No AI tool is an author.
...
the CAMB-in-the-loop inference drivers under \texttt{src/\allowbreak experiments/\allowbreak }
