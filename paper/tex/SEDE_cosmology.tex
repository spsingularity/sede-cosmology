...
\texttt{predictions.\allowbreak json}. The CPL \(w_{\mathrm{a}}\) steepens if fit
...
\texttt{fpab\_\allowbreak tensor\_\allowbreak sector.\allowbreak py}). Its perturbations are solved
...
\texttt{run\_\allowbreak model\_\allowbreak comparison.\allowbreak py} in the reproduction repository.
...
prediction set is committed as \texttt{predictions.\allowbreak json} in the
...
it is regenerated byte-for-byte by \texttt{gen\_\allowbreak predictions.\allowbreak py}, so the
...
symbolically in \texttt{fpab\_\allowbreak tensor\_\allowbreak sector.\allowbreak py} (regenerated on build).
...
\texttt{fig\_\allowbreak xi0\_\allowbreak w0.\allowbreak py} in the repository.
...
\texttt{tensor\_\allowbreak sector/\allowbreak prerecomb\_\allowbreak earlyde.\allowbreak py}, consistent with the
...
value throughout; script \texttt{run\_\allowbreak deficit\_\allowbreak data.\allowbreak py}). The background
is indistinguishable from \(\Lambda \mathrm{CDM}\) on H(z) alone: the
...
constraint invisible to data (script \texttt{run\_\allowbreak gate\_\allowbreak selection.\allowbreak py}).
On the accelerating attractor the gate's remaining fill decays at the
linear-growth rate \(\propto  e^{-2N}\) (N = ln a), slower than matter's
\(e^{-3N}\), so any gate still strictly filling at the attractor forces
H to undershoot \(H_\infty\) and the horizon entropy to overshoot its
ceiling transiently --- a sub-percent \((\approx 0.25\)\%), strictly
future \((a \gtrsim  6.5)\) violation of the generalized second law. No
theorem is contradicted: the null energy condition fails there
transiently, which is exactly where GSL proofs do not apply, and the
entire observable past is monotone. But for a model whose fundamental
object \emph{is} horizon entropy, requiring the second law globally is
natural, and it bounds the saturation endpoint: the gate must complete
its filling at x* ≲ 1.8, where
...
(\texttt{predictions.\allowbreak json}, verified byte-for-byte by
\texttt{gen\_\allowbreak predictions.\allowbreak py}). A single entry point
(\texttt{reproduce\_\allowbreak figures.\allowbreak py}) regenerates the figures, and the
inference drivers under \texttt{src/\allowbreak experiments/\allowbreak } regenerate the
